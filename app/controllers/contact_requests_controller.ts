...
export default class ContactRequestsController {
  ...
  /**
   * API pour obtenir les statistiques des demandes de contact
   */
  async apiStats({ response }: HttpContext) {
    try {
      const [totalToday, totalThisWeek, totalThisMonth, pendingCount, mostRequestedService] =
        await Promise.all([
      ...
      return response.json({
        success: true,
        data: {
          today: totalToday?.$extras.total || 0,
          thisWeek: totalThisWeek?.$extras.total || 0,
          thisMonth: totalThisMonth?.$extras.total || 0,
          pending: pendingCount?.$extras.total || 0,
          mostRequestedService: mostRequestedService?.service?.title || null,
        },
      })
    } catch (error) {
