              ...
              {users?.data && users.data.length > 0 ? (
                users.data.map((user: any) => (
                  <tr key={user.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${USER_ROLE_COLORS[user.role as UserRole]}`}>
                ...
                <tr className="text-center text-gray-500 py-8">
                  <td colSpan={6} className="px-6 py-8">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-xs text-gray-400">Les données seront chargées depuis le backend</p>
                    </div>
                  </td>
                </tr>
              )}
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Affichage de <span className="font-medium">{users?.meta ? ((users.meta.current_page - 1) * users.meta.per_page) + 1 : 0}</span> à{' '}
                  <span className="font-medium">{users?.meta ? Math.min(users.meta.current_page * users.meta.per_page, users.meta.total) : 0}</span> sur{' '}
                  <span className="font-medium">{users?.meta?.total || 0}</span> résultats
                </p>
              </div>
              <div>
